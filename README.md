...
```
This will take in all the ```dhdl.xvg``` as input and will calculate the free energies $ΔG_{NES-Total}$ and $ΔG_{NES-Solvent}$ using MBAR calculations.

The expected output of the command for this system is:

```RBFE = -1.97 +- 0.2 kcal/mol```

The following graphs will also be generated, and will be saved in the ```examples/rbfe_nes/sd_c3d_c5d/``` folder. These graphs show the histograms of the work values in the forward and reverse NES switches. 
