...
NES2_SCRIPT=hpc3/nes2.sh
[ -e $NES2_SCRIPT ] || fail "Run script $NES2_SCRIPT not found"
...
for main_system in "$@"; do
  echo "System $main_system"

  for ligand in A B; do
    system="${main_system}_${ligand}"
    ...
    all_stages=""
    [ "$ligand" == "A" ] && all_stages="1 2 3 4 5"
    [ "$ligand" == "B" ] && all_stages="5 6 7"
    ...
    for stage in $all_stages; do
      ...
      if [ "$stage" -eq 5 ]; then
        sbatch --dependency=afterok:"$jobidEq" --job-name="$system"-stage$stage-nes2 \
          --error="$SYSTEM_DIR"/slurm_output/nes2-s$stage-%a.err \
          --output="$SYSTEM_DIR"/slurm_output/nes2-s$stage-%a.out \
          --export=RUN_SCRIPT=$RUN_SCRIPT,SYSTEM_DIR="$SYSTEM_DIR",STAGE=$stage,RUN=$RUN \
          --array=1-$NUM_NES \
          $NES2_SCRIPT
      fi
