...
MINIMIZATION_SCRIPT=hpc3/minimization.sh
[ -e $MINIMIZATION_SCRIPT ] || fail "Run script $EQUILIBRIUM_SCRIPT not found"
...
for system in "$@"; do
  ...
  for stage in 1; do
    ...
    jobidEq=$(sbatch --parsable --job-name="$system"-stage$stage-minimization \
      --error="$SYSTEM_DIR"/slurm_output/minimize-s$stage.err \
      --output="$SYSTEM_DIR"/slurm_output/minimize-s$stage.out \
